...
class EditProfileImageView extends Component {
  ...
  _setCapturePhotoDetails = (image) => {
    if (image.mime == "image/png" || image.mime == "image/jpeg"
      ...
      if ((image.size / 1000000) > 10) {
        ...
        if (Platform.OS === 'ios') {
          getfileName = image.path.split('react-native-image-crop-picker/');
        }
        else{
          const splitdata = image.path.split('Pictures/');
          getfileName = splitdata[1];
        }
  ...
  TryUploadFile = async () => {
    ...
    if (Platform.OS === 'ios') {
      ...
      imagePath = `${RNFS.DocumentDirectoryPath}/${new Date().toISOString()}.jpg`.replace(/:/g, '-');
    ...
    RNFS.uploadFiles({
      ...
      if (res.Message === "success") {
        ...
        this.deleteFile();
        this.navigateToEditProfileImage();
      }
      else {
  ...
  render() {
    ...
    let { loading, profileimage } = this.props;
    const {button} =Resource_EN;
    const { uploading } = this.state;

    let profileimagedata = {};
    if (profileimage) {
      profileimagedata = profileimage.length > 0 ? profileimage[0] : {};
    }

    let profileImgPath = ApiConstants.SITEURL + ApiConstants.PROFILEIMGPATH + profileimagedata.customerimage;
    return (
        <View style={globalStyles.innerContainer}>
          {
            (get(loading, 'isLoading') || uploading) && <OverlayActivityIndicatorElement />
          }
          <Image style={EditProfileImageStyles.loginBg} source={require('../../assets/img/login_bg.jpg')} resizeMode="cover" /> 
          <ScrollView showsVerticalScrollIndicator={false} scrollEnabled={this.state.enableScroll}>
            <View style={EditProfileImageStyles.editprofileimageInner}>
              <View style={EditProfileImageStyles.ImgContainer}>
                {
                  profileimagedata.customerimage!= '' && profileimagedata.customerimage != null &&
                  <Image source={{ uri: profileImgPath }} resizeMode="contain" style={EditProfileImageStyles.profileImg} />
                }

                {
                  (profileimagedata.customerimage== '' || profileimagedata.customerimage == null) &&
                  <Image source={require('../../assets/img/user_img.png')} resizeMode="contain" style={EditProfileImageStyles.profileImg} />
                }
